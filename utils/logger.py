...
def export_log_to_pdf(user_email):
    """Converts user's JSON log into a PDF while handling special characters."""
    ...
    pdf.cell(200, 10, txt="VC Analyst AI: Research Report", ln=True, align='C')
    pdf.ln(10)
    ...
    if os.path.exists(json_filename):
        with open(json_filename, "r") as f:
            logs = json.load(f)
            for entry in logs:
                content = entry['content'].encode('latin-1', 'ignore').decode('latin-1')
                timestamp = entry['timestamp']
                role = entry['role'].upper()

                pdf.set_font("Arial", 'B', 10)
                pdf.multi_cell(0, 5, txt=f"[{timestamp}] {role}:")
